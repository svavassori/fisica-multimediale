...
The original software, that was written in 1997, cannot run on today's JRE and
Windows or macOS operating systems without changes, so it can be considered as
...
The simple solution is to call the getMinimumSize() instead of the deprecated method.
...
Since with Java 9 `java.awt.peer.ComponentPeer` class is not accessible and its
...
calling instead `java.util.concurrent.atomic.AtomicBoolean` to properly end the
animation.
