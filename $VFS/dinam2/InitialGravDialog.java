 ...
 */
public class InitialGravDialog extends TrackedDialog
  { 
    Component target;
    HelpDisplayer help;
    ...
    static final double vel_scale=1000;


    /**
     * Create a new settings dialog. If the user confirms
     * the changes, the target component will be notified with
     * an action event having a SimulationGrav object as the argument
     */
    public InitialGravDialog(Component target, 
                             Settings settings,
                             SimulationGrav initial,
                             double x, double y)
      { super(UserInterface.getDummyFrame(), 
        ...
        Label lab=new Label("Velocità iniziale [Km/s]");
        lab.setFont(font_large);
        vel_panel.add("", lab);

        vel=new NewVectorInput(-400, 400, -400, 400, 1000);
        if (initial!=null)
          vel.setValue(initial.getInitialVelX()/vel_scale, 
                       initial.getInitialVelY()/vel_scale);
        vel_panel.add("", vel);
        ...
        mass_panel.add("", lab);
        //massa=new NumericInput(1, 500, 1, 10);
        massa=new ExtNumInput(0.5, 0.01, 2.0E+27);

        if (initial!=null)
          massa.setValue(initial.getMass());
        mass_panel.add("", massa);

        lab=new Label("Posizione X [10^6 m]");
        lab.setFont(font_medium);
        mass_panel.add("", lab);
        ...
        lab=new Label("Posizione Y [10^6 m]");
        lab.setFont(font_medium);
        mass_panel.add("", lab);
    ...
    public boolean action(Event evt, Object what)
      { if ("OK".equals(what))
          { dispose();
            SimulationGrav simul=
              new SimulationGrav(settings,
                                 massa.getValue(),
                                 getValue(xPos),
                                 getValue(yPos),
                                 vel.getX()*vel_scale,
          ...
          { help.displayHelp("help/dinam2", "initgrav");
          }
