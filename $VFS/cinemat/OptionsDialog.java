package cinemat;

import ui.*;
import util.*;
import java.awt.*;


/**
 * A dialog box to input simulation options
 */
public class OptionsDialog extends TrackedDialog
  { 
    Component target;
    HelpDisplayer help;
    static Font font_small=new Font("TimesRoman", Font.PLAIN, 12);
    static Font font_medium=new Font("TimesRoman", Font.PLAIN, 14);
    static Font font_large=new Font("TimesRoman", Font.BOLD, 16);


    NumericInput duration;
    NumericInput stepDuration;
    NumericInput timeWarp;



    /**
     * Create a new settings dialog. If the user confirms
     * the changes, the target component will be notified with
     * an action event having an Options  object as the argument
     */
    public OptionsDialog(Component target, 
                             Options initial)
      { super(UserInterface.getDummyFrame(), 
              "Opzioni della simulazione", true);


        this.target=target;
        help=UserInterface.getHelpDisplayer();


        setLayout(new BorderLayout());

        Label lab;

        
        //
        // Crea il pannello con l'input di durata e velocita'
        //
        Panel panel=new Panel();
        panel.setLayout(new VerticalLayout(VerticalLayout.LEFT));

        lab=new Label("Durata della simulazione [s]");
        lab.setFont(font_medium);
        panel.add("", lab);
        duration=new NumericInput(1, 100, 60, 1);
        if (initial!=null)
          duration.setValue(initial.duration);
        panel.add("", duration);

        lab=new Label("Intervallo di visualizzazione valori [s]");
        lab.setFont(font_medium);
        panel.add("", lab);
        stepDuration=new NumericInput(1, 20, 8, 0.5);
        if (initial!=null)
          stepDuration.setValue(initial.stepDuration);
        panel.add("", stepDuration);

        lab=new Label("Velocità di simulazione");
        lab.setFont(font_medium);
        panel.add("", lab);
        timeWarp=new NumericInput(1, 25, 5, 0.2);
        if (initial!=null)
          timeWarp.setValue(initial.timeWarp);
        panel.add("", timeWarp);



        add("Center", panel);


        //
        // Crea la barra di bottoni in basso
        //
        ImageLoader il=UserInterface.getImageLoader();
        Panel confirm_panel=new Panel();
        confirm_panel.setFont(font_large);
        confirm_panel.setLayout(new FlowLayout(FlowLayout.LEFT));
        confirm_panel.add("", new ImageButton3D("OK",
                                   il.load("icons/ok.gif")));
        confirm_panel.add("", new ImageButton3D("Annulla",
                                   il.load("icons/cancel.gif")));
        confirm_panel.add("", new ImageButton3D("Aiuto",
                                   il.load("icons/help.gif")));
        // add("", confirm_panel);
        add("South", confirm_panel);

      }



    synchronized public boolean action(Event evt, Object what)
      { if ("OK".equals(what))
          { dispose();
            Options options=new Options();
            options.duration=duration.getValue();
            options.stepDuration=stepDuration.getValue();
            options.timeWarp=timeWarp.getValue();

            target.deliverEvent(new Event(target, Event.ACTION_EVENT, options));
          }
        else if ("Annulla".equals(what))
          { dispose();
          }
        else if ("Aiuto".equals(what))
          { help.displayHelp("help/cinemat", "options");
          }

        return true;
      }

    synchronized public boolean handleEvent(Event evt)
      { if (evt.id == Event.WINDOW_DESTROY)
          dispose();

        return super.handleEvent(evt);
      }
  }
