...
/**
 * A dialog box to input the value of a capacitor
 */
public class CondensatoreDialog extends TrackedDialog
  { 
    Component target;
    HelpDisplayer help;
     ...
     * an action event having a Condensatore object as the argument
     */
    public CondensatoreDialog(Component target)
      { super(UserInterface.getDummyFrame(), 
              "Aggiungi Condensatore", true);
        ...
        Label lab=new Label("Aggiungi un condensatore");
        lab.setFont(font_large);
        panel.add("", lab);

        C=new NumericField(11, 1e-15, 1);
        C.setValue(1e-6);
        panel.add(new LabeledComponent(C, "Capacità [F]", 
                                LabeledComponent.LEFT));

        Q0=new NumericField(11, 0, 0.1);
        Q0.setValue(0);
        panel.add(new LabeledComponent(Q0, "Carica iniziale [C]", 
                                LabeledComponent.LEFT));
    ...
    public boolean action(Event evt, Object what)
      { if ("OK".equals(what))
          ...
          { help.displayHelp("help/circuiti", "condensatore");
          }
