...
public class SettingsDialog extends TrackedDialog
  { 
    ...
    TabsPanel tabs;
    CheckboxGroup cbg_base, cbg_rel;
    Checkbox cb_base[], cb_rel[], cbAxes;
    NumericInput vel_base, acc_base;
    NumericInput vel_ang_base, raggio;
    VectorInput vel_rel;
    NumericInput vel_rel_ang;
    NumericInput x0;
    Label l_vel_base, l_acc_base;
    Label l_vel_ang_base, l_raggio;
    ...
    public SettingsDialog(Component target, Settings initial)
      {
        ...
        if (initial.ForcedSimulation==1)                                                                    
        {setTitle("Impostazione dei parametri - Moto Rettilineo Uniforme");}
                else if (initial.ForcedSimulation==2){setTitle("Impostazione dei parametri - Moto Rettilineo Uniformemente Accelerato");}
                else if (initial.ForcedSimulation==3){setTitle("Impostazione dei parametri - Moto Circolare");}
                else if (initial.ForcedSimulation==4){setTitle("Impostazione dei parametri - Moto Armonico");}
        ...
        //
        tabs=new TabsPanel();
        tabs.setVisible(initial.ForcedSimulation==0);
        ...
        tipo_panel.setVisible(initial.ForcedSimulation==0);
        ...
        l_vel_base=new Label("Moto traslatorio: velocità iniziale [m/s]");
        l_vel_base.setFont(font_medium);
        moto_panel.add("", l_vel_base);
        vel_base=new NumericInput(-200, 200, 0, 0.05);
        vel_base.setValue(initial.vel_base);
        moto_panel.add("", vel_base);

        l_acc_base=new Label("Moto traslatorio: accelerazione [m/s^2]");
        l_acc_base.setVisible(initial.ForcedSimulation!=1);
        l_acc_base.setFont(font_medium);
        moto_panel.add("", l_acc_base);
        acc_base=new NumericInput(-200, 200, 0, 0.025);
        acc_base.setValue(initial.acc_base);
        moto_panel.add("", acc_base);


        l_vel_ang_base=new Label("Moto rotatorio: velocità angolare [rad/s]");
        l_vel_ang_base.setFont(font_medium);
        moto_panel.add("", l_vel_ang_base);
        vel_ang_base=new NumericInput(-200, 200, 0, 0.025);
        vel_ang_base.setValue(initial.vel_ang_base);
        moto_panel.add("", vel_ang_base);

        l_raggio=new Label("Moto rotatorio: raggio [m]");
        l_raggio.setFont(font_medium);
        moto_panel.add("", l_raggio);
        raggio=new NumericInput(1, 100, 1, 0.1);
        raggio.setValue(initial.raggio);
        moto_panel.add("", raggio);

        if (initial.ForcedSimulation==3)
        {
         cbAxes= new Checkbox("Visualizza Proiezioni");
         cbAxes.setState(initial.ShowProjection);
         moto_panel.add("", cbAxes);
        ...
        vel_rel_panel.setVisible(initial.ForcedSimulation==0);
        ...
        rel_rot_panel.setVisible(initial.ForcedSimulation==0);
    ...
    synchronized public boolean action(Event evt, Object what)
      { if ("OK".equals(what))
          { dispose();
            ...
            if (cbAxes!=null)
            { settings.ShowProjection=cbAxes.getState(); }
            // .................................................................
            // omettendo questa riga, alla prossima comparsa del settings dialog
            // sar possibile scegliere un diverso tipo di esperimento not forced
               settings.ForcedSimulation=initial.ForcedSimulation;
            //..................................................................
    ...
    void checkCheckboxes()
      { int base=0;
        ...
        vel_base.setVisible(base==Settings.TRASLATORIO);
        acc_base.setVisible(base==Settings.TRASLATORIO && initial.ForcedSimulation!=1);
        vel_ang_base.setVisible(base==Settings.ROTATORIO);
        raggio.setVisible(base==Settings.ROTATORIO);
        l_vel_base.setVisible(base==Settings.TRASLATORIO);
        l_acc_base.setVisible(base==Settings.TRASLATORIO && initial.ForcedSimulation!=1);
        l_vel_ang_base.setVisible(base==Settings.ROTATORIO);
        l_raggio.setVisible(base==Settings.ROTATORIO);
