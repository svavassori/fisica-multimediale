 ...
 * for motion in an elastic field
 */
public class InitialElastDialog extends TrackedDialog
  { 
    Component target;
    HelpDisplayer help;
     ...
     * an action event having a SimulationElast object as the argument
     */
    public InitialElastDialog(Component target, 
                             Settings settings,
                             SimulationElast initial,
                             double x, double y)
      { super(UserInterface.getDummyFrame(), 
        ...
        mass_panel.add("", lab);
        massa=new ExtNumInput(0.5, 0.01, 2.0E27);
        if (initial!=null)
    ...
    public boolean action(Event evt, Object what)
      { if ("OK".equals(what))
          { dispose();
            SimulationElast simul=
              new SimulationElast(settings,
                                 massa.getValue(),
                                 getValue(xPos),
                                 getValue(yPos),
                                 vel.getXPos(),
          ...
          { help.displayHelp("help/dinam2", "initelas");
          }
