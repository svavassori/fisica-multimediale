...
public class SimulationDisplay extends SpriteCanvas implements Runnable
  {  double win_x1, win_y1, win_x2, win_y2;
     ...
     static final int defaultMarkers=25;
     Image ball;
     int ball_id;
     ...
     public SimulationDisplay(StatusDisplayer status,
       ...
       { 
         ...
         insets.top=fm.getHeight()+10;
         insets.bottom=fm.getHeight()+10;
         insets.left=3*fm.charWidth('A')+10;
         insets.right=3*fm.charWidth('A')+10;
...
//          thread.stop();
     ...
     public void run()
       { 
         ...
         int frames=(int)Math.ceil(options.duration/frameDelay)+1;
         if (frames<2)
           frames=2;

         simulation.run(frames*stepsPerFrame); 
         int s, n=simulation.getStepCount();
         int stepsBetweenUpdates=(frames/defaultMarkers)*stepsPerFrame;
         synchronized (this)
           { step=0;
           }
         recomputeWindow();
         ...
         for(s=0; s<n && isRunning.get(); s+=stepsPerFrame)
           { if (s%stepsBetweenUpdates == 0)
               { simulationInfoDisplay.update(s);
                 addMarker(simulation.getX(s), simulation.getY(s));
    ...
    void paintAxes(Graphics g, Rectangle clip)
      { Dimension d=size();
        ...
        axesDrawer.drawXAxis(g, zero.y, clip);
