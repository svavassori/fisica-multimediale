...
/**
 * A canvas displaying the refraction animation
 */
public class Refract extends Canvas 
  ...
  { static final int ONDE_PIANE=Settings.ONDE_PIANE;
    ...
    Settings settings;
    RefractSettings refractSettings=null;
    ...
    public void set(RefractSettings refractSettings)
      { this.refractSettings=refractSettings;
        recomputeWindow();
        step=0;
        repaint();
        legenda.repaint();
    ...
    public void paint(Component com, Graphics g)
      { if (refractSettings==null)
        ...
        g.drawString("Onda rifratta", 100, h*2+asc+3);
        if (trueRefraction())
          { double n=refractSettings.indice;
            double l1=settings.vel*settings.periodo;
            double l2=l1/n;
            g.drawString("lungh. d'onda: "+Format.format(".3", l1)+" m",
                         d.width/2+20, h+asc+3);
            g.drawString("lungh. d'onda: "+Format.format(".3", l2)+" m",
                         d.width/2+20, h*2+asc+3);
          }
        else
          { 
            double l1=settings.vel*settings.periodo;
            g.drawString("lungh. d'onda: "+Format.format(".3", l1)+" m",
                         d.width/2+20, h+asc+3);
            g.drawString("l'onda viene riflessa",
                         d.width/2+20, h*2+asc+3);
...
//          thread.stop();
    ...
    void recomputeWindow()
      { side=settings.vel*settings.periodo*6;
        if (settings.tipo==ONDE_CIRCOLARI && refractSettings!=null)
          { 
            int o=refractSettings.onda;
            double x=settings.x[o];
            double y=settings.y[o];
            double z[]={0, 0};
            imagePoint(x, y, z);
            double x1=z[0];  // x della sorg. rifratta
            double y1=z[1];  // y della sorg. rifratta 

            imagePointReflected(x, y, z);
            double x2=z[0]; // x della sorg. riflessa
            double y2=z[1]; // y della sorg. riflessa
            ...
            double xa=Math.min(Math.min(x, x1), x2)-side/8;
            double xb=Math.max(Math.max(x, x1), x2)+side/8;
            double ya=Math.min(Math.min(y, y1), y2)-side/8;
            double yb=Math.max(Math.max(y, y1), y2)+side/8;
            double xc=(xa+xb)/2;
            double yc=(ya+yb)/2;
            if (xa<xc-side)
              xa=xc-side;
    ...
    public void paint(Graphics g)
      { super.paint(g);
        ...
        if (refractSettings!=null)
          { double ang=refractSettings.angolo*Math.PI/180;
    ...
    /**
     * Disegna il raggio incidente e il raggio rifratto 
     * per le onde piane.
     */
    void paintRays(Graphics g)
      { int o=refractSettings.onda;
        ...
        double ang1=refractAngle(ang);
        double x1=3*side*Math.cos(ang1);
        double y1=3*side*Math.sin(ang1);
        double norm=refractSettings.angolo*Math.PI/180+Math.PI/2;
        double nx=3*side*Math.cos(norm);
        double ny=3*side*Math.sin(norm);
    ...
    /**
     * Disegna le sorgenti dell'onda incidente e dell'onda rifratta
    ...
    void imagePoint(double x, double y, double z[])
      { double ang=refractSettings.angolo*Math.PI/180;
        ...
        double cos=Math.cos(ang);
        double n=refractSettings.indice;
        double h=cos*x+sin*y;
        double xx=h*cos;
        double yy=h*sin;
        double r=Functions.hypot(x-xx, y-yy);
        double r1=r*n;
        if (r<1e-7)
           r=1e-7;
        double x1=xx+r1/r*(x-xx);
        double y1=yy+r1/r*(y-yy);
    ...
    /**
     * Calcola il punto riflesso di un punto assegnato
     */
    void imagePointReflected(double x, double y, double z[])
      { double ang=refractSettings.angolo*Math.PI/180;
        double sin=Math.sin(ang);
        double cos=Math.cos(ang);
        double h=cos*x+sin*y;
    ...
    boolean paintFront(Graphics g, int st, boolean last, int dir)
      { Rectangle old=clip(g);
        ...
        // raggio rifratto
        double alpha1=refractAngle(alpha);
        double s1=Math.sin(alpha1);
        double c1=Math.cos(alpha1);

        // fronte incidente
        double q=(c*sin-s*cos)*(c*cos+s*sin);
        double beta=alpha+Functions.sign(q)*Math.PI/2;
        if (Functions.sign(q)==0)
          beta=alpha+Math.PI/2; 
        double fs=Math.sin(beta);
        double fc=Math.cos(beta);

        // fronte rifratto
        double q1=(c1*sin-s1*cos)*(c1*cos+s1*sin);
        double beta1=alpha1-Functions.sign(q1)*Math.PI/2;
        if (Functions.sign(q1)==0)
        ...
        if (!before)
          { double t=Functions.hypot(x, y);
            if (trueRefraction())
              t/=refractSettings.indice;
            x=t*c1;
            y=t*s1;
            fsx=fs1;
            fcx=fc1;
    ...
    boolean paintFront(Graphics g, int st, boolean last, double x, double y)
      { double ang=refractSettings.angolo*Math.PI/180;
        ...
        double f=(360-settings.fase[refractSettings.onda])%360*Math.PI/180;
        ...
        if (r<=h)
          ...
          { double n=refractSettings.indice;
            ...
            double r1=h*n+(r-h)/n;
            double delta1=Math.acos(h*n/(r1*0.99));
            ...
            g.drawArc(pt1.x, pt1.y, pt2.x-pt1.x, pt2.y-pt1.y, 
                      ang2, 360-(ang2-ang1));
            ang1=(int)Math.floor((alpha-delta1)*180/Math.PI+0.5);
            ang2=(int)Math.floor((alpha+delta1)*180/Math.PI+0.5);
            g.setColor(last? Color.red: Color.orange);
            pt1=realToPixel(x1-r1, y1+r1);
            pt2=realToPixel(x1+r1, y1-r1);
            if (ang2-ang1>1)
              g.drawArc(pt1.x, pt1.y, pt2.x-pt1.x, pt2.y-pt1.y, 
                      ang1, ang2-ang1);
            out=pt2.x-pt1.x>size().width*1.4;
     ...
     * @param ang angolo in radianti
     */
    double refractAngle(double ang)
      { double n=refractSettings.indice;
        double ang_plane=refractSettings.angolo*Math.PI/180;
        double sin=Math.sin(ang_plane);
        double cos=Math.cos(ang_plane);
        double s=Math.sin(ang);
        double c=Math.cos(ang);
        double norm;
        if (cos*s-sin*c>=0)
          norm=ang_plane+Math.PI/2;
        else
          norm=ang_plane-Math.PI/2;
        double diff=ang-norm;
        double ds=Math.sin(diff);
        double ds1=ds/n;  
        if (Math.abs(ds1)>1) // Riflessione
          return 2*ang_plane-ang;
        else // Rifrazione
          return norm+Math.asin(ds1);
      }


    /**
     * Controlla se c'e' vera rifrazione, o se invece il raggio e' riflesso
     */
    boolean trueRefraction()
      { if (settings.tipo==ONDE_PIANE)
          {
            double n=refractSettings.indice;
            double ang_plane=refractSettings.angolo*Math.PI/180;
            double sin=Math.sin(ang_plane);
            double cos=Math.cos(ang_plane);
            double ang=settings.dir[refractSettings.onda]*Math.PI/180;
            double s=Math.sin(ang);
            double c=Math.cos(ang);
            double ss=Math.abs(cos*c+sin*s);
            return ss/n <= 1;
          }
        else // ONDE_CIRCOLARI
          { return true;
