...
/**
 * Dialog box per l'input dei param. dell'effetto compton
 */
public class ComptonDialog extends TrackedDialog
  { SimulationCompton target;
    HelpDisplayer help;
    ...
    NumericField lambda, ang;

    /**
     * Create a new dialog.  If the user confirms
     * the choices, the target component will be notified by
     * means of the startAnimation(double,double) method. 
     */
    public ComptonDialog(SimulationCompton target)
      { super(UserInterface.getDummyFrame(), 
        ...
        Label lab=new Label("Compton dell'atomo di idrogeno");
        lab.setFont(font_large);
        panel.add("", lab);

        lambda=new NumericField(8,1e-3,1e4);
        lambda.setValue(1);
        panel.add("", new LabeledComponent(lambda, 
                                "Lungh. d'onda del fotone incidente [A]",
                                           LabeledComponent.LEFT));

        ang=new NumericField(8,1,180);
        ang.setValue(45);
        panel.add("", new LabeledComponent(ang, 
                                "Angolo di deflessione del fotone [deg]",
                                           LabeledComponent.LEFT));
    ...
    public boolean action(Event evt, Object what)
      { if ("OK".equals(what))
          { dispose();
            target.startAnimation(lambda.getValue(), ang.getValue());
          }
        else if ("Annulla".equals(what))
          { dispose();
          }
        else if ("Aiuto".equals(what))
          { help.displayHelp("help/fismod", "compton");
          }
        return true;
      }
