 ...
 * for plane constrained motion
 */
public class InitialPlaneDialog extends TrackedDialog
  { 
    Component target;
    HelpDisplayer help;
     ...
     * an action event having a SimulationPlane object as the argument
     */
    public InitialPlaneDialog(Component target, 
                             Settings settings,
                             SimulationPlane initial,
                             double x, double y)
      { super(UserInterface.getDummyFrame(), 
        ...
        lab=new Label("Posizione (ascissa curvilinea) [m]");
        lab.setFont(font_medium);
        panel.add("", lab);
        pos=new TextField(8);
        pos.setText(Format.format(".3", p));
        panel.add("", pos);

        lab=new Label("Velocità iniziale [m/s]");
        lab.setFont(font_medium);
        panel.add("", lab);
        vel=new NumericInput(-80, 80, 0, 0.25);
        if (initial!=null)
    ...
    public boolean action(Event evt, Object what)
      { if ("OK".equals(what))
          { dispose();
            SimulationPlane simul=
              new SimulationPlane(settings,
                                 massa.getValue(),
                                 getValue(pos),
                                 vel.getValue());
          ...
          { help.displayHelp("help/dinam1", "initplan");
          }
