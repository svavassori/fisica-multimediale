...
/**
 * Simulazione del moto senza vincoli
 * @author Pasquale Foggia
 * @version 0.99, Dec 1997
 */
public class SimulationFree extends Simulation implements ODE
  { 
    double vx[], vy[];
    double ax[], ay[];
    ...
    public SimulationFree(Settings settings, 
                          double mass,
                          double x0, double y0, 
   ...
   public double getInitialVelY()
          ...
          { double Ax=settings.fx/mass;
            ...
            for(i=0; i<steps; i++)
              { ax[i]=Ax;
                ...
                vx[i]=vx00+Ax*t[i];
                vy[i]=vy00+Ay*t[i];
                x[i]=x00+vx00*t[i]+0.5*Ax*t[i]*t[i];
                y[i]=y00+vy00*t[i]+0.5*Ay*t[i]*t[i];
              }
          }
        else
          { 
            ...
            for(i=0; i<steps; i++)
              { int status=interpolator.interpolate(t[i], yy);
                ...
                vx[i]=yy[2];
                vy[i]=yy[3];
                computeAcceleration(vx[i], vy[i], yy);
                ax[i]=yy[0];
                ay[i]=yy[1];
              }
          }
    ...
    public void derive(double t, double yy[], double yy1[])
      { yy1[0]=yy[2];
        yy1[1]=yy[3];
        double zz[]={0,0};
        computeAcceleration(yy[2], yy[3], zz);
        yy1[2]=zz[0];
        yy1[3]=zz[1];
      }

    public void computeAcceleration(double vx, double vy, double a[])
